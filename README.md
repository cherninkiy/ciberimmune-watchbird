...
:white_check_mark: __Дополнительное задание__

# Переработанная политика архитектуры

Ключевые моменты:
- произведена декомпозиция центральной системы управления (модуль ЭЦП, модуль рассчета маршрута, модуль управления нейтрализатором)
- модуль рассчета маршрута рассчитывает маршрут с учетом данных навигации и уровня заряда батареи, которые он получает напрямую от соответствующих подсистем (уровень заряда батареи все же учитываю, есть риски повреждения объектов инфраструктуры или утраты дрона)
- модуль самодиагоностики получает данные подсистем и отвечает за отправку их в систему планирования полетов и ОрВД

Так получилось, что объединил таблицы 2 и 3, сделал сравнение по версиям.

| Компонент | Архитектура v0.00 | Архитектура v0.01 | 
| :-- | :-- | :-- |
| 1. Связь |  Отвечает за взаимодействие с системой распределения полётов и системой организации воздушного движения (ОрВД) <br> 🔴 <span style="color:red">Недоверенный</span> (CL) | *... без изменений* <br> 🔴 <span style="color:red">Недоверенный</span> (CL) |
| 2. Сбор и анализ данных | Собирает и анализирует данные фото-видеофиксации <br> 🔴 <span style="color:red">Недоверенный</span> (ML) | *... без изменений* <br> 🔴 <span style="color:red">Недоверенный</span> (ML) |
| 3. Центральная система управления | Осуществляет общее управление дроном, контролирует выполнение полётного задания <br> 🟡 <span style="color:orange">Повышающий доверие</span> (CXL) | **декомпозирован** |
| 10. Модуль ЭЦП | | - проверка ЭЦП полетного задания<br> 🟡 <span style="color:orange">Повышающий доверие</span> (SS) <br> *Обоснование: возможны нарушения __ЦБ1__, __ЦБ2__* |
| 11. Модуль рассчета маршрута | | - рассчет маршрута и параметров перемещения<br>- проверка прибытия в заданный район/на базу<br>- проверка завершения миссии<br> 🟡 <span style="color:orange">Повышающий доверие</span> (MM) <br> *Обоснование: возможно нарушение __ЦБ3__* |
| 12. Модуль управления нейтрализатором | | - обработка результатов мониторинга<br>- подача команд для рассчета маршрута маневрирования<br>- управление нейтрализатором <br> 🔴 <span style="color:red">Недоверенный</span> (MM) *Обоснование: __ЦБ1__-__ЦБ3__ не нарушаются* |
| 4. Навигация | Предоставляет спутниковые координаты дрона <br> 🟡 <span style="color:orange">Повышающий доверие</span> (SS) | *... без изменений* <br> 🟡 <span style="color:orange">Повышающий доверие</span> (SS) |
| 5. Нейтрализатор | Отвечает за физическое воздействие на правонарушителя <br> 🔴 <span style="color:red">Недоверенный</span> (SS) | *... без изменений* <br> 🔴 <span style="color:red">Недоверенный</span> (SS) |
| 6. Полётный контроллер | Осуществляет комплекс действий для поддержания дрона в воздухе и его перемещения в заданном направлении <br> 🟢 <span style="color:green">Доверенный</span> (ML) | *... без изменений* <br> 🟢 <span style="color:green">Доверенный</span> (ML) |
| 7. Самодиагностика | Предоставляет данные оценки состояния подсистем (например, контроля батареи) <br> 🟢 <span style="color:green">Доверенный</span> (SM) | *... без изменений* <br> 🔴 <span style="color:red">Недоверенный</span> (SM) <br>*Обоснование: __ЦБ1__-__ЦБ3__ не нарушаются*|
| 8. Приводы | Серво-приводы и драйверы для их управления - управляют скоростью вращения винтов дрона, механизацией вспомогательных крыльев (если есть) 🟢 <span style="color:green">Доверенный</span> (SS) | *... без изменений* <br> 🟢 <span style="color:green">Доверенный</span> (SS) |
| 9. Контроль батареи | Предоставляет статус остаточного заряда батареи и оценку продолжительности полёта в текущем режиме энергопотребления <br>🟢 <span style="color:green">Доверенный</span> (SS) | *... без изменений* <br> 🟢 <span style="color:green">Доверенный</span> (SS) |

## Политика архитектуры v0.01

![Политика архитектуры v0.01](images/страж-птица-Политика%20архитектуры%20v0.01.svg)

Изменения:
- сложный повышающий доверие компонет (центральная система управления) декомпозирован на 3 более простых, из которых один недоверенный!
- на 1 доверенный компонент меньше (самодиагностика)

Количество компонентов версии 0.01:
- 🔴 недоверенные компоненты 5/11
- 🟢 доверенные (🟡 повышающие доверие) компоненты 6/11
...
__(Политика архитектуры v0.00)__

![Политика архитектуры v0.00](images/страж-птица-Политика%20архитектуры%20v0.00.drawio.svg)


## Базовый сценарий + HLA v0.01

![Базовый сценарий + HLA v0.01](images/страж-птица-Базовый%20сценарий%20+%20HLA%20v0.01.svg)

[Код диаграммы](http://www.plantuml.com/plantuml/uml/h5JDJjje5DtxAQxOGaOJvf_5Q6OtYtc4R4nYm6gmezY9r1s45gY2YgYwgAgsg9lkAfa4AoOauHNkzqOztv5n79UABge8udo-zznppxdV_IUAtMxSssyxiH-tFLdJZ-POKvrhOWuqqnlHJtfHhzd_lyYw_FV_lskEu_RYCEZjRtjTfuDql-btt22soPgPGpC0m4oJHg1tUgi9m3A0BNIKFv-lYHj9ouywRjpS-mha0u9VwMi6xORzrRVrdmHLHkPS9-QC8Sqm26Grw0y69Jf3qMlDy3H6uGMfPJebjKpu6dnmk62pSnqJCVBQEotVtQrK_XcexooYET9RSoRw6QfSCgFlTVqTlzf9lRwQqGXG9Z67-9neQWRaCRDAcrFy7TaJmdN3Nbm5-nLOxz7924J7o3akX0pSYaP_8VIjxSqCg3l9NY5dG5TJpPYtxSQeydGrzq_aNbAD4OaFSqMw_J2ggFox3INy1P3dPgWtw6TQBenFQWu8OMUcuxV3kABHPbKZVDyX6vtQREN007UEvnlY9ZZFTC9-5gmpOuLEDuoy9ifKbDlaD